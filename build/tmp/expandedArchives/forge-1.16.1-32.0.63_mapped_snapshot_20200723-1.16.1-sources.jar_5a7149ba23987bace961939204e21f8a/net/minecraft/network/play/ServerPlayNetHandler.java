...
public class ServerPlayNetHandler implements IServerPlayNetHandler {
   ...
   public void handleRecipeBookUpdate(CRecipeInfoPacket packetIn) {
      ...
      if (packetIn.getPurpose() == CRecipeInfoPacket.Purpose.SHOWN) {
         ...
         this.player.getRecipeBook().setBlastFurnaceGuiOpen(packetIn.func_218779_h());
         this.player.getRecipeBook().setBlastFurnaceFilteringCraftable(packetIn.func_218778_i());
         this.player.getRecipeBook().setSmokerGuiOpen(packetIn.func_218780_j());
         this.player.getRecipeBook().setSmokerFilteringCraftable(packetIn.func_218781_k());
      }
      ...
      if (entity != null) {
         ...
         if (this.player.getDistanceSq(entity) < 36.0D) {
            ...
            if (optional.isPresent() && optional.get().isSuccessOrConsume()) {
               CriteriaTriggers.PLAYER_ENTITY_INTERACTION.test(this.player, this.player.getHeldItem(hand), entity);
   ...
   public void processClientStatus(CClientStatusPacket packetIn) {
      ...
      case PERFORM_RESPAWN:
         if (this.player.queuedEndExit) {
            ...
            CriteriaTriggers.CHANGED_DIMENSION.testForAll(this.player, World.field_234920_i_, World.field_234918_g_);
         } else {
