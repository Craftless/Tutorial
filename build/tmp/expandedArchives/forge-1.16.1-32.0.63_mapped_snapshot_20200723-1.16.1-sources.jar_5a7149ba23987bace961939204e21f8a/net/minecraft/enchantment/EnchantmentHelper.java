...
public class EnchantmentHelper {
   ...
   public static Map<Enchantment, Integer> deserializeEnchantments(ListNBT serialized) {
      ...
      for(int i = 0; i < serialized.size(); ++i) {
         ...
         Registry.ENCHANTMENT.getValue(ResourceLocation.tryCreate(compoundnbt.getString("id"))).ifPresent((enchantment) -> {
   ...
   private static void applyEnchantmentModifier(EnchantmentHelper.IEnchantmentVisitor modifier, ItemStack stack) {
      if (!stack.isEmpty()) {
         ...
         for(int i = 0; i < listnbt.size(); ++i) {
            ...
            Registry.ENCHANTMENT.getValue(ResourceLocation.tryCreate(s)).ifPresent((enchantment) -> {
