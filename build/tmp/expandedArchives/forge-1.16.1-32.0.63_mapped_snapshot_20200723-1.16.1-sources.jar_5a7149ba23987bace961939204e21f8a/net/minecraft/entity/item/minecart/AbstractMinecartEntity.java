   ...
   public Vector3d func_230268_c_(LivingEntity livingEntity) {
      ...
      if (direction.getAxis() == Direction.Axis.Y) {
         ...
         for(Pose pose : immutablelist) {
            ...
            for(int i : field_234627_am_.get(pose)) {
               for(int[] aint1 : aint) {
                  ...
                  double d0 = this.world.func_234932_c_(blockpos$mutable, (state) -> {
         ...
         for(Pose pose1 : immutablelist) {
            ...
            double d3 = (double)blockpos$mutable.getY() + this.world.func_234928_a_(blockpos$mutable, d1 - (double)blockpos$mutable.getY() + d2);
            if (d1 + d2 <= d3) {
