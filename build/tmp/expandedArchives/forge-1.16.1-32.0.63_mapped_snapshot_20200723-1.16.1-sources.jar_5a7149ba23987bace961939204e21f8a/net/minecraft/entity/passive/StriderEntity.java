...
public class StriderEntity extends AnimalEntity implements IRideable, IEquipable {
   ...
   private void func_234318_eL_() {
      if (this.isInLava()) {
         ...
         if (iselectioncontext.func_216378_a(FlowingFluidBlock.field_235510_c_, this.getPosition(), true) && !this.world.getFluidState(this.getPosition().up()).isTagged(FluidTags.LAVA)) {
            this.onGround = true;
         } else {
   ...
   public ILivingEntityData onInitialSpawn(IWorld worldIn, DifficultyInstance difficultyIn, SpawnReason reason, @Nullable ILivingEntityData spawnDataIn, @Nullable CompoundNBT dataTag) {
      ...
      if (spawnDataIn instanceof StriderEntity.AgeData) {
         ...
         ((AgeableEntity.AgeableData)spawnDataIn).setBabySpawnProbability(striderentity$agedata$ridertype == StriderEntity.AgeData.RiderType.NO_RIDER ? 0.5F : 0.0F);
      } else {
         striderentity$agedata$ridertype = StriderEntity.AgeData.RiderType.NO_RIDER;
      }
