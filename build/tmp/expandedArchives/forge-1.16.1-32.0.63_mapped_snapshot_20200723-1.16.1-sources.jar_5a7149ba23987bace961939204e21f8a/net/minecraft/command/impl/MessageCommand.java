...
public class MessageCommand {
   ...
   private static int sendPrivateMessage(CommandSource source, Collection<ServerPlayerEntity> recipients, ITextComponent message) {
      ...
      for(ServerPlayerEntity serverplayerentity : recipients) {
         serverplayerentity.sendMessage((new TranslationTextComponent("commands.message.display.incoming", source.getDisplayName(), message)).mergeStyle(new TextFormatting[]{TextFormatting.GRAY, TextFormatting.ITALIC}), uuid);
         source.sendFeedback((new TranslationTextComponent("commands.message.display.outgoing", serverplayerentity.getDisplayName(), message)).mergeStyle(new TextFormatting[]{TextFormatting.GRAY, TextFormatting.ITALIC}), false);
      }
