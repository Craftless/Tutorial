   ...
   private FullscreenResolutionOption(MainWindow mainWindowIn, @Nullable Monitor monitorIn) {
      super("options.fullscreen.resolution", -1.0D, monitorIn != null ? (double)(monitorIn.getVideoModeCount() - 1) : -1.0D, 1.0F, (p_225306_2_) -> {
         ...
         if (monitorIn == null) {
            ...
            return d0 == -1.0D ? iformattabletextcomponent.append(new TranslationTextComponent("options.fullscreen.current")) : iformattabletextcomponent.appendString(monitorIn.getVideoModeFromIndex((int)d0).toString());
         }
      });
   }
}