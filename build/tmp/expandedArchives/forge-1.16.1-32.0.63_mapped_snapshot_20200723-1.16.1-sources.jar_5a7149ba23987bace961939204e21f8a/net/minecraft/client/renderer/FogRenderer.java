...
public class FogRenderer {
   ...
   public static void updateFogColor(ActiveRenderInfo activeRenderInfoIn, float partialTicks, ClientWorld worldIn, int renderDistanceChunks, float bossColorModifier) {
      ...
      if (fluidstate.isTagged(FluidTags.WATER)) {
         ...
         float f12 = MathHelper.clamp(MathHelper.cos(worldIn.getCelestialAngle(partialTicks) * ((float)Math.PI * 2F)) * 2.0F + 0.5F, 0.0F, 1.0F);
         ...
         Vector3d vector3d2 = CubicSampler.func_240807_a_(vector3d1, (p_239218_3_, p_239218_4_, p_239218_5_) -> {
            return worldIn.func_239132_a_().func_230494_a_(Vector3d.unpack(biomemanager.func_235199_a_(p_239218_3_, p_239218_4_, p_239218_5_).func_235080_i_()), f12);
         });
         ...
         if (renderDistanceChunks >= 4) {
            ...
            if (f17 > 0.0F) {
               float[] afloat = worldIn.func_239132_a_().func_230492_a_(worldIn.getCelestialAngle(partialTicks), partialTicks);
               if (afloat != null) {
                  f17 = f17 * afloat[3];
