...
public class CompassItem extends Item implements IVanishable {
   ...
   public void inventoryTick(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
      if (!worldIn.isRemote) {
         if (func_234670_d_(stack)) {
            ...
            if (optional.isPresent() && optional.get() == worldIn.func_234923_W_() && compoundnbt.contains("LodestonePos") && !((ServerWorld)worldIn).getPointOfInterestManager().func_234135_a_(PointOfInterestType.LODESTONE, NBTUtil.readBlockPos(compoundnbt.getCompound("LodestonePos")))) {
   ...
   public ActionResultType onItemUse(ItemUseContext context) {
      ...
      if (!context.world.getBlockState(blockpos).isIn(Blocks.LODESTONE)) {
         return super.onItemUse(context);
      } else {
         context.world.playSound((PlayerEntity)null, blockpos, SoundEvents.ITEM_LODESTONE_COMPASS_LOCK, SoundCategory.PLAYERS, 1.0F, 1.0F);
