...
public class RedstoneWireBlock extends Block {
   ...
   public RedstoneWireBlock(AbstractBlock.Properties properties) {
      ...
      for(BlockState blockstate : this.getStateContainer().getValidStates()) {
         if (blockstate.get(POWER) == 0) {
            this.field_235541_j_.put(blockstate, this.getShapeForState(blockstate));
         }
      }
   ...
   public BlockState getStateForPlacement(BlockItemUseContext context) {
      return this.getUpdatedState(context.getWorld(), this.field_235543_o_, context.getPos());
   }

   private BlockState getUpdatedState(IBlockReader reader, BlockState state, BlockPos pos) {
      boolean flag = func_235556_n_(state);
      state = this.func_235551_b_(reader, this.getDefaultState().with(POWER, state.get(POWER)), pos);
      if (flag && func_235556_n_(state)) {
         return state;
      } else {
   ...
   public BlockState updatePostPlacement(BlockState stateIn, Direction facing, BlockState facingState, IWorld worldIn, BlockPos currentPos, BlockPos facingPos) {
      if (facing == Direction.DOWN) {
         ...
         return redstoneside.func_235921_b_() == stateIn.get(FACING_PROPERTY_MAP.get(facing)).func_235921_b_() && !func_235555_m_(stateIn) ? stateIn.with(FACING_PROPERTY_MAP.get(facing), redstoneside) : this.getUpdatedState(worldIn, this.field_235543_o_.with(POWER, stateIn.get(POWER)).with(FACING_PROPERTY_MAP.get(facing), redstoneside), currentPos);
      }
   }
      ...
      for(Direction direction : Direction.Plane.HORIZONTAL) {
         ...
         if (redstoneside != RedstoneSide.NONE && !worldIn.getBlockState(blockpos$mutable.func_239622_a_(pos, direction)).isIn(this)) {
            ...
            if (!blockstate.isIn(Blocks.OBSERVER)) {
               ...
               func_241468_a_(blockstate, blockstate1, worldIn, blockpos$mutable, flags, recursionLeft);
            }
            ...
            if (!blockstate3.isIn(Blocks.OBSERVER)) {
               ...
               func_241468_a_(blockstate3, blockstate2, worldIn, blockpos$mutable, flags, recursionLeft);
            }
         }
      }
   ...
   private RedstoneSide func_235545_a_(IBlockReader reader, BlockPos pos, Direction direction, boolean p_235545_4_) {
      BlockPos blockpos = pos.offset(direction);
      BlockState blockstate = reader.getBlockState(blockpos);
      if (p_235545_4_) {
         boolean flag = this.func_235552_b_(reader, blockpos, blockstate);
      ...
      if (i < 15) {
         for(Direction direction : Direction.Plane.HORIZONTAL) {
            ...
            if (blockstate.isNormalCube(world, blockpos) && !world.getBlockState(blockpos1).isNormalCube(world, blockpos1)) {
               j = Math.max(j, this.func_235557_o_(world.getBlockState(blockpos.up())));
            } else if (!blockstate.isNormalCube(world, blockpos)) {
               j = Math.max(j, this.func_235557_o_(world.getBlockState(blockpos.down())));
            }
         }
      }
   ...
   @OnlyIn(Dist.CLIENT)
   private void func_235549_a_(World world, Random rand, BlockPos pos, Vector3f p_235549_4_, Direction p_235549_5_, Direction p_235549_6_, float p_235549_7_, float p_235549_8_) {
      float f = p_235549_8_ - p_235549_7_;
      if (!(rand.nextFloat() >= 0.2F * f)) {
         float f1 = 0.4375F;
         float f2 = p_235549_7_ + f * rand.nextFloat();
         ...
         world.addParticle(new RedstoneParticleData(p_235549_4_.getX(), p_235549_4_.getY(), p_235549_4_.getZ(), 1.0F), (double)pos.getX() + d0, (double)pos.getY() + d1, (double)pos.getZ() + d2, 0.0D, 0.0D, 0.0D);
      }
   }
