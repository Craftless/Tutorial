...
public class BlockPattern {
   ...
   public static class PatternHelper {
      ...
      public BlockPattern.PortalInfo getPortalInfo(Direction directionIn, BlockPos pos, double portalY, Vector3d vector, double portalXZ) {
         Direction direction = this.getForwards();
         Direction direction1 = direction.rotateY();
         double d1 = (double)(this.getFrontTopLeft().getY() + 1) - portalY * (double)this.getHeight();
         double d0;
         double d2;
         if (direction1 == Direction.NORTH) {
            d0 = (double)pos.getX() + 0.5D;
            d2 = (double)(this.getFrontTopLeft().getZ() + 1) - (1.0D - portalXZ) * (double)this.getWidth();
         } else if (direction1 == Direction.SOUTH) {
            d0 = (double)pos.getX() + 0.5D;
            d2 = (double)this.getFrontTopLeft().getZ() + (1.0D - portalXZ) * (double)this.getWidth();
         } else if (direction1 == Direction.WEST) {
            d0 = (double)(this.getFrontTopLeft().getX() + 1) - (1.0D - portalXZ) * (double)this.getWidth();
            d2 = (double)pos.getZ() + 0.5D;
         } else {
            d0 = (double)this.getFrontTopLeft().getX() + (1.0D - portalXZ) * (double)this.getWidth();
            d2 = (double)pos.getZ() + 0.5D;
         }

         double d3;
         double d4;
         if (direction.getOpposite() == directionIn) {
            d3 = vector.x;
            d4 = vector.z;
         } else if (direction.getOpposite() == directionIn.getOpposite()) {
            d3 = -vector.x;
            d4 = -vector.z;
         } else if (direction.getOpposite() == directionIn.rotateY()) {
            d3 = -vector.z;
            d4 = vector.x;
         } else {
            d3 = vector.z;
         ...
         int i = (direction.getHorizontalIndex() - directionIn.getOpposite().getHorizontalIndex()) * 90;
         return new BlockPattern.PortalInfo(new Vector3d(d0, d1, d2), new Vector3d(d3, vector.y, d4), i);
      }
   }
   ...
   public static class PortalInfo {
      ...
      public PortalInfo(Vector3d pos, Vector3d motion, int rotation) {
         this.pos = pos;
         this.motion = motion;
