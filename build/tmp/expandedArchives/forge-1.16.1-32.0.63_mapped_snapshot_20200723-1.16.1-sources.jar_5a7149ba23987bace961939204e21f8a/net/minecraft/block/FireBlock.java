...
public class FireBlock extends AbstractFireBlock {
   ...
   public VoxelShape getShape(BlockState state, IBlockReader worldIn, BlockPos pos, ISelectionContext context) {
      ...
      if (state.get(WEST)) {
         voxelshape = VoxelShapes.or(voxelshape, shapeWest);
      }

      if (state.get(EAST)) {
         voxelshape = VoxelShapes.or(voxelshape, shapeEast);
      }

      if (state.get(NORTH)) {
         voxelshape = VoxelShapes.or(voxelshape, shapeNorth);
      }

      if (state.get(SOUTH)) {
         voxelshape = VoxelShapes.or(voxelshape, shapeSouth);
      }

      return voxelshape == VoxelShapes.empty() ? shapeDown : voxelshape;
   }
   ...
   private static int func_235495_a_(Random rand) {
