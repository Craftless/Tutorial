...
public abstract class AbstractFireBlock extends Block {
   ...
   protected static final VoxelShape shapeUp = Block.makeCuboidShape(0.0D, 15.0D, 0.0D, 16.0D, 16.0D, 16.0D);
   protected static final VoxelShape shapeDown = Block.makeCuboidShape(0.0D, 0.0D, 0.0D, 16.0D, 1.0D, 16.0D);
   protected static final VoxelShape shapeWest = Block.makeCuboidShape(0.0D, 0.0D, 0.0D, 1.0D, 16.0D, 16.0D);
   protected static final VoxelShape shapeEast = Block.makeCuboidShape(15.0D, 0.0D, 0.0D, 16.0D, 16.0D, 16.0D);
   protected static final VoxelShape shapeNorth = Block.makeCuboidShape(0.0D, 0.0D, 0.0D, 16.0D, 16.0D, 1.0D);
   protected static final VoxelShape shapeSouth = Block.makeCuboidShape(0.0D, 0.0D, 15.0D, 16.0D, 16.0D, 16.0D);
   ...
   public static boolean canLightBlock(IWorld world, BlockPos pos) {
      BlockState blockstate = world.getBlockState(pos);
      BlockState blockstate1 = getFireForPlacement(world, pos);
      return blockstate.isAir() && (blockstate1.isValidPosition(world, pos) || shouldLightPortal(world, pos));
   }

   private static boolean shouldLightPortal(IWorld world, BlockPos pos) {
      for(Direction direction : Direction.Plane.HORIZONTAL) {
         if (world.getBlockState(pos.offset(direction)).isIn(Blocks.OBSIDIAN) && NetherPortalBlock.isPortal(world, pos) != null) {
            return true;
         }
      }
