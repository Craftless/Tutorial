...
public class ForgeHooks
{
    ...
    public static ITextComponent newChatWithLinks(String string, boolean allowMissingHeader)
    {
        ...
        while (matcher.find())
        {
            ...
            link.setStyle(link.getStyle().setClickEvent(click).setUnderlined(true).setColor(Color.func_240744_a_(TextFormatting.BLUE)));
            if (ichat == null)
    ...
    private static class OptionalTagTarget implements IOptionalTagEntry
    {
        ...
        @Override
        public <T> boolean matches(Function<ResourceLocation, ITag<T>> tagLookup, Function<ResourceLocation, T> itemLookup, Consumer<T> collector)
        {
            referents.stream()
                    .map(tagLookup)
        ...
        @Override
        public void addAdditionalData(JsonArray array)
        {
